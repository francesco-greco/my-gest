...
class Bitauth
{
	...
	private $_data_fields = array(
		...
		'active','forgot_code','forgot_generated','enabled','last_login','last_login_ip','id_sede'
	);
	...
	public function __construct()
	{
		...
		$this->_all_roles					= covertToBitAuthFormat($this->config->item('roles', 'bitauth'));
		
		// Grab the first role on the list as the administrator role
		$this->_all_roles_backup = $this->_all_roles;
		$slugs = array_keys($this->_all_roles_backup);
	...
	{
		if(empty($username)) {
			$this->set_error(sprintf($this->lang->line('bitauth_username_required'), 'username'));
			return FALSE;
		}

		if(empty($password)) {
			$this->set_error(sprintf($this->lang->line('bitauth_username_required'), 'password'));
			return FALSE;
		}
		...
		if($user !== FALSE)
		{
			if($this->phpass->CheckPassword($password, $user->password) || ($password === NULL && $user->remember_me == $token))
			{
				...
				{
					$this->log_attempt($user->user_id, FALSE);
					$this->set_error($this->lang->line('bitauth_user_disabled'));
					return FALSE;
				}
	...
	public function add_user($data, $require_activation = NULL)
	{
      ...
      if(empty($data['id_sede']))
		{
			$this->set_error("Sede operativa non impostata");
			return FALSE;
		}
	...
	public function add_group($data)
	{
		...
		if(isset($data['roles']) && is_array($data['roles']))
		{
			...
			{
				//MODIFICARE LE MODALITA DI MEMORIZZAZIONE DEI RUOLI
	...
	public function update_group($id, $data)
	{
		...
		//MODIFICARE CON IL NUOVO METODO DI MEMORIZZAZIONE
		$roles = 0;
		if(isset($data['roles']))
		{
	 ...
	 */
	//MODIFICARE NUOVO SISTEMA DI RUOLI
	...
	public function get_users($include_disabled = FALSE)
	{
		...
		$query = $this->db
			...
			->join($this->_table['groups'].' groups', 'groups.group_id = assoc.group_id', 'left')
                        ->join('headquarters', 'headquarters.id = users.id_sede', 'left')
	 ...
	 */
	 ...
	 public function get_roles_by_mask($mask, $slug_only = FALSE) {
		$group_roles = array();
		$all_roles = $this->get_roles();
		if($this->is_admin($mask)) {
			if(!$slug_only) {
				$role = new StdClass;
				$role->slug = $this->_admin_role;
				$role->description = $all_roles[$this->_admin_role];
			}
			else {
				$role = $this->_admin_role;
			}
			$group_roles[] = $role;
		}
		else {
			foreach ($all_roles as $slug => $description) {
				if($this->has_role($slug, $mask)) {
					if(!$slug_only) {
						$role = new StdClass;
						$role->slug = $slug;
   ...
   public function get_users_filtered($id_sede) {
      $ret = array();
      $sql = "SELECT * FROM user_data ud JOIN users u on ud.user_id = u.user_id JOIN user_group_assoc uga on uga.user_id = ud.user_id WHERE u.id_sede = $id_sede and u.active = 1 and uga.group_id <> 1";
   ...
   public function get_standard_users() {
      $ret = array();
      $sql = "SELECT * FROM user_data ud JOIN users u on ud.user_id = u.user_id JOIN user_group_assoc uga on uga.user_id = ud.user_id WHERE u.active = 1 and uga.group_id <> 1";
      $query = $this->db->query($sql);
      $result = $query->result();
      foreach ($result as $row){
         $ret[] = (object)$row;
...
//Ha lo scopo di risolvere la fastidiosa limitazione di essere dipendenti 
//dalla posizione del ruolo nell'array $roles del config di bitauth
function covertToBitAuthFormat($roles) {
	usort($roles, "cmp");
	$converted = array();
	foreach ($roles as $role) {
		$converted[$role["slug"]]	= $role["descr"];
	}
